...
- 红黑树的出现，1.8 中当桶中链表元素长度达到8 个时，链表则会转成红黑树，让 O(N) 访问效率转为O(logN)。当红黑树上的元素个数，减少到6个时，就退化为链表
...
因为(2的n次幂 - 1)都是11111结尾的，所以碰撞几率小。使Hash算法的结果均匀分布。这样计算之后， 在 n 为 2 ^ n 时， 其实相当于 hash % n，& 当然比 % 效率高
...
[面试官再问你 HashMap 底层原理，就把这篇文章甩给他看](https://mp.weixin.qq.com/s/KgG8wUMJU-zzOgWOL51ivA)
